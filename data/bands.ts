const bands = [
  {
    collectionId: "APPLE_WATCH_SERIES_10",
    band: [
      // side-42-titanium-natural
      {
        id: "stainless_steel",
        name: "Stainless Steel",
        variations: [
          // side-42-titanium-natural-stainless_steel-natural_milanese
          {
            id: "natural_milanese",
            name: "Natural Milanese Loop",
            price: 129,
            image: "/images/bands/milanese_natural_10.jpg",
          },
          // side-42-titanium-natural-stainless_steel-gold_milanese
          {
            id: "gold_milanese",
            name: "Gold Milanese Loop",
            price: 139,
            image: "/images/bands/milanese_gold_10.jpg",
          }, 
        ],
      },
      {
        id: "sport_loop",
        name: "Sport Loop",
        variations: [
          // side-42-titanium-natural-sport_loop-ultramarine
          {
            id: "ultramarine",
            name: "Ultramarine Sport Loop",
            price: 49,
            image: "/images/bands/sport_ultramarine_10.jpg",
          },
          // side-42-titanium-natural-sport_loop-lake_green
          {
            id: "lake_green",
            name: "Lake Green Sport Loop",
            price: 49,
            image: "/images/bands/sport_lake_green_10.jpg",
          }, 
        ],
      },
      
      {
        id: "solo_loop",
        name: "Solo Loop",
        variations: [
          // side-42-titanium-natural-solo_loop-black
          {
            id: "black",
            name: "Black Solo Loop",
            price: 79,
            image: "/images/bands/solo_black_10.jpg",
          },
          // side-42-titanium-natural-solo_loop-ligh_blush
          {
            id: "ligh_blush",
            name: "Light Blush Solo Loop",
            price: 79,
            image: "/images/bands/solo_light_blush_10.jpg",
          },
        ],
      },
    ],
  },
  {
    collectionId: "APPLE_WATCH_HERMÈS_SERIES_10",
    band: [
      // side-46-titanium-silver
      {
        id: "hermes_toile_h",
        name: "Hermès Toile H",
        variations: [
          // side-46-titanium-silver-hermes_toile_h-toile_hermes
          {
            id: "toile_hermes",
            name: "Gold/Écru Toile H Single Tour",
            price: 149,
            image: "/images/bands/toile_single_tour_hermes.jpg",
          },
        ],
      },
      {
        id: "hermes_grand_h",
        name: "Hermès Grand H",
        variations: [
           // side-46-titanium-silver-hermes_grand_h-satine_hermes
          {
            id: "satine_hermes",
            name: "Satiné Grand H",
            price: 650,
            image: "/images/bands/satine_grand_hermes.jpg",
          },
        ],
      },
    ],
  },
  {
    collectionId: "APPLE_WATCH_SE",
    band: [
      // side-44-aluminum-silver
      {
        id: "stainless_steel",
        name: "Stainless Steel",
        variations: [
          // side-44-aluminum-silver-stainless_steel-natural_milanese
          {
            id: "natural_milanese",
            name: "Natural Milanese Loop",
            price: 129,
            image: "/images/bands/milanese_natural_10.jpg",
          }, 
        ],
      },
      {
        id: "sport_loop",
        name: "Sport Loop",
        variations: [
          // side-44-aluminum-silver-sport_loop-lake_green
          {
            id: "lake_green",
            name: "Lake Green Sport Loop",
            price: 49,
            image: "/images/bands/sport_lake_green_10.jpg",
          },
        ],
      },
      {
        id: "solo_loop",
        name: "Solo Loop",
        variations: [
          // side-44-aluminum-silver-solo_loop-star_fruit
          {
            id: "star_fruit",
            name: "Star Fruit Solo Loop",
            price: 69,
            image: "/images/bands/solo_star_fruit_10.jpg",
          },
          // side-44-aluminum-silver-solo_loop-ultramarine
          {
            id: "ultramarine",
            name: "Ultramarine Solo Loop",
            price: 69,
            image: "/images/bands/solo_ultramarine_10.jpg",
          },
         
        ],
      }
    ],
  },
];

export default bands;
